...
export default function AppetizersSection({ categories }) {
  ...
  const APPETIZERS_CATEGORY_ID = "690ac3d4e762b2dc267f2f48";
    ...
    fetch(`${backend_url}/foods`)
      ...
      .then((data) => {
        ...
        const appetizers = allFoods.filter(
          (food) => food.category?._id === APPETIZERS_CATEGORY_ID,
        );
        setFoods(appetizers);
      })
      .catch((err) => {
  ...
  const handleAddDish = async () => {
    ...
    const newFood = {
      ...
      category: APPETIZERS_CATEGORY_ID,
    };
    ...
    try {
      ...
      if (res.ok) {
        ...
        const appetizers = (refreshData.foods || []).filter(
          (food) => food.category?._id === APPETIZERS_CATEGORY_ID,
        );
        setFoods(appetizers);
        setShowAddModal(false);
        showNotification("New dish added to Appetizers!", "black");
      } else {
  ...
  const refetchFoods = async () => {
    try {
      const res = await fetch(`${backend_url}/foods`);
      const data = await res.json();
      const appetizers = (data.foods || []).filter(
        (food) => food.category?._id === APPETIZERS_CATEGORY_ID,
      );
      setFoods(appetizers);
    } catch (err) {
